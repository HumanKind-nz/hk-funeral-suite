	...
	add_action( 'admin_head', function () use ( $post_type, $svg_icon ) {
		...
		<style>
			...
			#adminmenu .menu-icon-hk_fs_<?php echo esc_attr( $post_type ); ?>.current div.wp-menu-image::before {
				background-image: url('data:image/svg+xml;utf8,<?php echo $hover_svg; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- rawurlencode()d SVG written in this file. ?>');
				opacity: 1;
			}
		</style>
		<?php
