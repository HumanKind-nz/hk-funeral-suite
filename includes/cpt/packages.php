 ...
 if (!function_exists('hk_fs_cpt_register_packages')) {
	 function hk_fs_cpt_register_packages() {
		 ...
		 $args = array(
			 ...
			 'hierarchical'        => false,
			 // phpcs:ignore WordPress.PHP.DiscouragedPHPFunctions.obfuscation_base64_encode -- Inline SVG menu icon, the WordPress-documented form.
...
function hk_fs_package_intro_callback($post) {
 ...
 <p>
	 <input type="text" id="hk_fs_package_intro" name="hk_fs_package_intro" 
			...
			placeholder="<?php esc_html_e('Enter brief summary to be shown at top ...', 'hk-funeral-cpt'); ?>">
	 <span class="description"><?php esc_html_e('Enter an optional introductory sentence for this pricing package.', 'hk-funeral-cpt'); ?></span>
 </p>
 <?php
}
...
function hk_fs_package_pricing_callback($post) {
	...
	<div class="price-field-container <?php echo $managed_by_sheets ? 'sheet-managed' : ''; ?>">
		<p>
			<label for="hk_fs_package_price"><?php esc_html_e('Price ($):', 'hk-funeral-cpt'); ?></label>
			...
			<span class="description" style="font-size: 11px; color: #757575;">
				<?php esc_html_e('Enter a numeric price (e.g., 1295.00) or text like "P.O.A."', 'hk-funeral-cpt'); ?>
			</span>
		</p>
...
function hk_fs_package_ordering_callback($post) {
	...
	?>
	<p>
		<label for="hk_fs_package_order"><?php esc_html_e('Display Order:', 'hk-funeral-cpt'); ?></label>
	...
	if ($managed_by_sheets) {
		...
		<div class="notice notice-info">
			<p>
				...
				<?php esc_html_e('Package pricing is currently managed via Google Sheets. Price fields are disabled in the admin interface.', 'hk-funeral-cpt'); ?>
