	...
	foreach ($blocks as $block) {
		if ($block['blockName'] === 'hk-funeral-suite/keepsake') {
			...
			if (!$managed_by_sheets && isset($attrs['price']) && $attrs['price'] !== '') {
				...
				$existing_price = get_post_meta($post_id, '_hk_fs_keepsake_price', true);
				if (empty($existing_price)) {
					// Only clear if there's no existing value
					update_post_meta($post_id, '_hk_fs_keepsake_price', '');
				}
			...
			// Save memorial type to post meta
			if (isset($attrs['memorialType'])) {
				update_post_meta($post_id, '_hk_fs_keepsake_memorial_type', sanitize_text_field($attrs['memorialType']));
			}
			
			// Save personalization to post meta
			if (isset($attrs['personalization'])) {
				update_post_meta($post_id, '_hk_fs_keepsake_personalization', sanitize_text_field($attrs['personalization']));
			}
...
function hk_fs_load_keepsake_block_data() {
	...
	$meta_values = array(
		...
		'metalType' => get_post_meta($post->ID, '_hk_fs_keepsake_metal_type', true),
	...
	if (wp_script_is('hk-fs-keepsake-block', 'registered')) {
